...
const BlogItem = React.memo(function BlogItem({
  ...
  const highlightText = useCallback((text: string, query?: string) => {
    ...
    return parts.map((part, i) =>
      regex.test(part) ? (
        <mark
          key={i}
          className="bg-green-900/60 text-green-400 px-0.5 rounded-2xl bg-transparent"
        >
          {part}
        </mark>
      ) : (
  ...
  return (
    <div className="mb-4 p-5 rounded-2xl hover:border-[#666666] transition-colors">
      <Link href={`/home-blog/${post.id}`} className="group block">
        <h3 className="text-lg text-[#e0e0e0] mb-2 font-bold leading-tight group-hover:text-blue-400 transition-colors">
    ...
    <div className="p-4 sm:p-5 border border-[#444444] bg-[#181818] rounded-4xl hover:border-2 hover:border-blue-500 transition-colors">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="inline-flex items-center px-2 py-0.5 rounded-full bg-blue-800 text-white text-sm w-fit">
            {feedback.category}
          </span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-2 mb-4 text-sm">
        <div className="flex items-center gap-2 min-w-0">
          ...
          <span className="text-white font-bold shrink-0">Name</span>
        ...
        {feedback.github_id && (
          <div className="flex items-center gap-2 min-w-0 sm:col-span-2">
            ...
            <span className="text-white font-bold shrink-0">GitHub</span>
            <a
              href={`https://github.com/${feedback.github_id}`}
              target="_blank"
              rel="noopener noreferrer"
              className="text-white hover:text-green-400 truncate transition-colors"
            >
              {feedback.github_id}
            </a>
          </div>
          ...
          {availableTags.map((tag) => {
            ...
            return (
              <button
                ...
                className={`inline-flex items-center justify-center px-3 py-1 text-sm rounded-full cursor-pointer transition-colors ${
                  isSelected
                    ? "bg-blue-400 text-black font-semibold border-blue-500 border-2"
                    : "bg-[#222222] text-white hover:border-blue-500 border-[#444444] border-2 "
                }`}
              >
                {tag}
              </button>
